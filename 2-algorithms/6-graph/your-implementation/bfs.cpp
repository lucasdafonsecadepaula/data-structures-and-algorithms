...
bool bfs(vector<vector<int>> adjList, int src, int dst, vector<int> &path)
{
}
...
{
    {
        ...
        vector<int> path;
        assert(bfs(graph, 0, 0, path) == false);
        assert(path.empty());
    }

    {
        vector<vector<int>> graph = {{}};
        vector<int> path;
        assert(bfs(graph, 0, 0, path) == true);
...

    {
        ...
        bool result = bfs(graph, 0, 3, path);
        assert(result == true);
...

    {
        ...
        vector<int> path;
        assert(bfs(graph, 0, 2, path) == false);
    }

    {
        ...
        bool result = bfs(graph, 0, 2, path);
        assert(result == true);
        assert(path == heuristc);
    }
...

    {
        ...
        bool result = bfs(graph, 0, 5, path);
        assert(result == true);
        assert(path == heuristc);
    }
}
